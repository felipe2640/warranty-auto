...
import { addTicketAttachment, addTicketTimelineEntry } from "@/lib/services/warrantyService"
import { getUserPermissions } from "@/lib/permissions"
import { AttachmentCategoryEnum } from "@/lib/schemas"
import { getTicketById, getTicketAttachmentById } from "@/lib/repositories/tickets"
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!permissions.canAddTimeline) {
      return errorResponse("FORBIDDEN", "Permissão insuficiente para adicionar timeline", 403)
    }
    ...
    const attachment = await getTicketAttachmentById(id, result.attachmentId)
    const entryText = text?.trim() || `Documento anexado: ${attachment?.name ?? file.name}`

    const timelineEntryId = await addTicketTimelineEntry({
      ticketId: id,
      tenantId: session.tenantId,
      entry: {
        type: "DOCUMENTO",
        ...
        attachmentDriveFileId: attachment?.driveFileId,
    ...
    return NextResponse.json({ success: true, attachmentId: result.attachmentId, timelineEntryId })
  } catch (error) {
    console.error("Add timeline attachment error:", error)
