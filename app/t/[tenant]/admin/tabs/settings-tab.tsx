...
export function SettingsTab({ settings, onRefresh }: SettingsTabProps) {
  ...
  const [recebedorOnlyOwnStore, setRecebedorOnlyOwnStore] = useState(
    settings.policies.recebedorOnlyOwnStore
  );
  const [requireCanhotForCobranca, setRequireCanhotForCobranca] = useState(
    settings.policies.requireCanhotForCobranca
  );
  const [allowCloseWithoutResolution, setAllowCloseWithoutResolution] =
    useState(settings.policies.allowCloseWithoutResolution);
  const [defaultSlaDays, setDefaultSlaDays] = useState(
    String(settings.policies.defaultSlaDays || 30)
  );
  ...
  const handleTestDrive = async () => {
    if (!driveRootFolderId) {
      setTestResult({
        success: false,
        message: "Configure o ID da pasta primeiro",
      });
      return;
    }
    ...
    try {
      ...
      setTestResult({
        ...
        message:
          data.message ||
          (response.ok ? "Conexão bem sucedida!" : "Falha na conexão"),
      });
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure a integração com o Google Drive para armazenamento de
            arquivos
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              O ID pode ser encontrado na URL da pasta do Google Drive após
              /folders/
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              disabled={isTesting || !driveRootFolderId}
            >
              {isTesting ? (
                <>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between rounded-lg border p-4">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Limita a visibilidade de tickets para usuários com papel
                RECEBEDOR
              </p>
            </div>
            <Switch
              checked={recebedorOnlyOwnStore}
              onCheckedChange={setRecebedorOnlyOwnStore}
            />
          </div>
          ...
          <div className="flex items-center justify-between rounded-lg border p-4">
            ...
            <Switch
              checked={requireCanhotForCobranca}
              onCheckedChange={setRequireCanhotForCobranca}
            />
          </div>
          ...
          <div className="flex items-center justify-between rounded-lg border p-4">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Permite fechar tickets sem definir um resultado final
              </p>
            </div>
            <Switch
              checked={allowCloseWithoutResolution}
              onCheckedChange={setAllowCloseWithoutResolution}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Prazo padrão quando o fornecedor não tem SLA específico
            </p>
          </div>
        </CardContent>
      </Card>
